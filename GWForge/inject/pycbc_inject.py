...
class PyCBCInject:
    def __init__(
        ...
        for ifo in self.ifos:
            ...
            # bilby and pycbc use different azimuth conventions: pycbc's
            # add_detector_on_earth expects cartographer-style angles, so the bilby
            # arm azimuths are converted with (360 - azimuth) and the x/y arms are
            # exchanged. This conversion was verified to reproduce bilby's antenna
            # response |F+|^2 + |Fx|^2 for CE40 to ~5e-5.
    ...
    def get_pycbc_signal(self, ifo, signal_parameters):
        ...
        hplus, hcross = get_td_waveform(
            **signal_parameters,
            ...
            delta_t=1.0 / ifo.sampling_frequency,
        )
        ...
        signal = detector.project_wave(
            hplus,
            hcross,
            signal_parameters["ra"],
            signal_parameters["dec"],
            signal_parameters["psi"],
            method="lal",
            reference_time=signal_parameters["tc"],
        )
        return signal
    ...
    def inject_signal_using_pycbc_method(self):
        ...
        for ifo in self.ifos:
            ...
            for k in track(
                range(len(self.injection_parameters["mass1"])),
