...
class Spin:
    def __init__(self, spin_model, number_of_samples, parameters={}):
        """
        Parameters:
        ----------
        spin_model : str
    ...
    def sample(self):
        ...
        if self.spin_model in [utils.remove_special_characters(choice.lower()) for choice in choices]:
            pass
        else:
        ...
        if "nonspinning" in self.spin_model:
            if "gaussiannonspinning" in self.spin_model:
                ...
                chi_1 = bilby.core.prior.analytical.TruncatedGaussian(
                    name="chi_1", minimum=self.parameters["minimum_primary_spin"], maximum=self.parameters["maximum_primary_spin"], mu=mu_chi_1, sigma=sigma_chi_1
                )
            ...
            if "bilby" in self.spin_model or "aligned" == self.spin_model:
                chi_1 = bilby.gw.prior.AlignedSpin(name="chi_1", a_prior=bilby.gw.prior.Uniform(minimum=self.parameters["minimum_primary_spin"], maximum=self.parameters["maximum_primary_spin"]))
                chi_2 = bilby.gw.prior.AlignedSpin(name="chi_2", a_prior=bilby.gw.prior.Uniform(minimum=self.parameters["minimum_secondary_spin"], maximum=self.parameters["maximum_secondary_spin"]))
                ...
                chi_2 = bilby.gw.prior.Uniform(name="chi_2", minimum=self.parameters["minimum_secondary_spin"], maximum=self.parameters["maximum_secondary_spin"])
            elif "uniform" in self.spin_model:
                chi_1 = bilby.gw.prior.Uniform(name="chi_1", minimum=self.parameters["minimum_primary_spin"], maximum=self.parameters["maximum_primary_spin"])
                chi_2 = bilby.gw.prior.Uniform(name="chi_2", minimum=self.parameters["minimum_secondary_spin"], maximum=self.parameters["maximum_secondary_spin"])
            ...
            if "bilby" in self.spin_model:
                a_1 = bilby.gw.prior.Uniform(minimum=self.parameters["minimum_primary_spin"], maximum=self.parameters["maximum_primary_spin"], name="a_1")
                a_2 = bilby.gw.prior.Uniform(minimum=self.parameters["minimum_secondary_spin"], maximum=self.parameters["maximum_secondary_spin"], name="a_2")
                ...
                cos_tilt_gaussian = bilby.core.prior.analytical.TruncatedNormal(name="cos_tilt_gaussian", mu=1, sigma=self.parameters["sigma_t"], minimum=-1, maximum=1)
                ...
                cos_tilt = bilby.core.prior.analytical.TruncatedNormal(name="cos_tilt", mu=1, sigma=self.parameters["sigma_t"], minimum=-1, maximum=1)
